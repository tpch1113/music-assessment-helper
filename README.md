...
3. `AI 보조` 탭에서 PDF 파일을 업로드하거나, PDF에서 복사한 학생 작품 내용이나 텍스트를 직접 붙여넣습니다.
4. PDF 텍스트 추출에 실패하면 안내 문구에 따라 내용을 직접 복사해 넣습니다.
5. `AI 점수 추천`을 누르면 평가기준별 추천 점수와 이유가 표시됩니다.
6. `점수 적용`과 `피드백 적용`을 누른 뒤, `채점` 탭에서 교사가 최종 점수를 확인하고 수정합니다.
7. `저장 후 다음`을 눌러 결과를 저장합니다.

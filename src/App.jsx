...
const PDFLIB_SCRIPT_URL = 'https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js';
...
function loadPdfLibScript() {
  return new Promise((resolve, reject) => {
    ...
    const existingScript = document.querySelector(`script[src="${PDFLIB_SCRIPT_URL}"]`);
    if (existingScript) {
      existingScript.addEventListener('load', () => resolve(window.PDFLib), { once: true });
      existingScript.addEventListener('error', reject, { once: true });
      return;
    }

    const script = document.createElement('script');
    script.src = PDFLIB_SCRIPT_URL;
    script.async = true;
...
function App() {
  ...
  const saveMergedSubmissionPdf = async (basePdfArrayBuffer, attachedPdfs, fileName) => {
    if (attachedPdfs.length === 0) {
      const blob = new Blob([basePdfArrayBuffer], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = fileName;
    ...
    const { PDFDocument } = await loadPdfLibScript();
    const mergedPdf = await PDFDocument.create();
    const basePdf = await PDFDocument.load(basePdfArrayBuffer);
    ...
    for (const attachedPdf of attachedPdfs) {
      const sourcePdf = await PDFDocument.load(dataUrlToArrayBuffer(attachedPdf.dataUrl));
  ...
  const createStudentSubmissionPdf = async (
    ...
    pdfFiles = googleSubmissionPdfs
  ) => {
    if (!targetStudent?.name) {
      setExportStatus('PDF로 내보낼 학생을 선택해 주세요.');
      return;
    }
    if (images.length === 0 && pdfFiles.length === 0) {
      setExportStatus('PDF로 내보낼 제출 이미지 또는 PDF 첨부파일이 없습니다.');
      return;
    }
    ...
    try {
      ...
      await saveMergedSubmissionPdf(pdf.output('arraybuffer'), pdfFiles, fileName);
  ...
  const createClassSubmissionPdf = async () => {
    try {
      ...
      orderedStudents.forEach((item, studentIndex) => {
        ...
        if (pdfs.length > 0) hasAnyFile = true;
      });

      if (!hasAnyFile) {
        setExportStatus('현재 반에 PDF로 내보낼 제출 이미지 또는 PDF 첨부파일이 없습니다.');
        return;
      }
      ...
      await saveMergedSubmissionPdf(pdf.output('arraybuffer'), attachedPdfs, fileName);
      setExportStatus(`${fileName} 파일을 만들었습니다.`);
    } catch (error) {
